...
async def get_date_context(timezone_name: str = "") -> dict:
    timezone_name = (timezone_name or "").strip()
    if timezone_name and ZoneInfo is not None:
        try:
            localized_now = datetime.now(ZoneInfo(timezone_name))
        except Exception:
            localized_now = datetime.now()
    else:
        localized_now = datetime.now()
    # zhdate expects a naive datetime even when we first localize by device timezone.
    now = localized_now.replace(tzinfo=None) if getattr(localized_now, "tzinfo", None) else localized_now
...
async def get_date_context_cached(ttl: float = 900, timezone_name: str = "") -> dict:
    """Cached version of get_date_context (15min default TTL)."""
    cache_key = f"date_context:{(timezone_name or '').strip()}"
    cached = _cache_get(cache_key, ttl)
    if cached is not None:
        return cached
    result = await get_date_context(timezone_name=timezone_name)
