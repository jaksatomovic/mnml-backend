...
async def create_user(
    ...
    *,
    ...
    This function only inserts into the users table.
    It does not consume invitation codes or initialize quota.
    For strongly consistent registration flows, prefer a higher-level wrapper.
    """
    ...
    try:
        ...
        # Return None on username / phone / email uniqueness conflicts.
        return None
...
async def create_access_request(mac: str, requester_user_id: int) -> dict:
    ...
    now = datetime.now().isoformat()
    db = await get_main_db()
    request_id = await execute_insert_returning_id(
        db,
...
async def save_user_llm_config(
    ...
    # Add llm_access_mode if it does not exist yet.
