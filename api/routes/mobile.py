...
async def get_today_content(
    ...
    timezone_name = str((prefs or {}).get("timezone", "") or "").strip()
    date_ctx = await get_date_context_cached(timezone_name=timezone_name)
...
async def get_widget_data(
    ...
    if latest and latest.get("mode_id", "").upper() == selected_mode:
        ...
        timezone_name = str(effective_cfg.get("timezone", "") or "").strip()
        date_ctx = await get_date_context_cached(timezone_name=timezone_name)
